...
// Define the size of the view in world space
...
Vector2 ConvertScreenToWorld(Vector2 screen) {
    ...
    // Calculate the extents of the world space based on view size and zoom
    Vector2 extents = (Vector2){ ncViewSize * ratio, ncViewSize };
    extents = Vector2Scale(extents, ncScreenZoom);

    // Calculate lower and upper bounds of the world space
    ...
    // Map normalized screen coordinates to world coordinates within bounds
    Vector2 world;
...
// Function to convert world distance to pixel distance
float ConvertWorldToPixel(float world) {
    ...
    // Normalize world distance
    float nx = world / extents.x;
    // Map normalized world distance to pixel distance
